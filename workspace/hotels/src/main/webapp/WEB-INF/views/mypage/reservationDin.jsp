...
<!-- 서브헤더 끝  -->
	...
	<div class="hotelParadiseWrap">
		...
		<div class="list">
			<c:forEach var="rt" items="${rtL}">
			<div class="listHead">
				<p><span class="strong">접수일: ${rt.date_write} </span></p>
				<a href="/mypage/reservationDinCheck/${rt.no}"><span class="strong click">예약내용 상세보기</span></a>
			</div>
			<div class="listBody">
				<div class="listBodyLeft">
					<p><span class="strong">${rt.restaurant_name}</span></p>
					<p>${rt.date_reserve}</p>
				</div>
				<div class="listBodyRight">
					<p>${rt.time_meal}</p>
					<p><span id="check">${rt.state}</span></p>
				</div>
			</div>
			</c:forEach>
		</div>
